process_spotify_HITS <- function(HITS_files) {
  ...
  DF_all = readr::read_csv(HITS_files, id = "file", show_col_types = FALSE) |>
    ...
    tidyr::separate(file, into = c("date", "time", "list"), sep = " ")
  ...
  DF_out = DF_all |>
    ...
    dplyr::select(date, Cancion, Artista, source) |>
    dplyr::rename(
        song = Cancion,
        artists = Artista) |>
    ...
    dplyr::select(year, song, artists, source, song_Explicit)
